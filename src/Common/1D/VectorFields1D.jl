function _getdimensionalproperty(s::ScalarField{1}, sym::Symbol)
...
function _getdimensionalproperty(e::ElectricField{1}, sym::Symbol)
...
function Base.propertynames(::ScalarField{1}, private=false)
    if private
        return fieldnames(ScalarField)
    else
        return (:positions,:values,:left,:right)
    end
end
